...
bool ManualMapDll(HANDLE hProc, BYTE* pSrcData, SIZE_T FileSize, bool ClearHeader = true, bool ClearNonNeededSections = true, bool AdjustProtections = true, DWORD fdwReason = DLL_PROCESS_ATTACH, LPVOID lpReserved = nullptr) {
...
export bool inject(const std::filesystem::path& dll_path, const std::wstring& process_name) {
	auto dllPath = dll_path.wstring().c_str();
	...
	void* reserved = VirtualAllocEx(hProc, nullptr, current_path.size(), MEM_COMMIT, PAGE_EXECUTE_READWRITE);
	WriteProcessMemory(hProc, reserved, current_path.data(), current_path.size(), nullptr);
	...
	if (!std::filesystem::exists(dll_path)) {
		std::cout << "Dll path does not exist\n";
		return false;
	}
	...
	printf("Mapping...\n");
	if (!ManualMapDll(hProc, pSrcData, FileSize, true, true, true, DLL_PROCESS_ATTACH, reserved)) {
		delete[] pSrcData;
		CloseHandle(hProc);
