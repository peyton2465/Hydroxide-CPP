...
std::uintptr_t* g_methodsTable = NULL;
static kiero::RenderType::Enum g_renderType = kiero::RenderType::None;

export namespace kiero {
	Status::Enum init(RenderType::Enum _renderType) {
		if (g_renderType != RenderType::None) {
			return Status::AlreadyInitializedError;
		}

		if (_renderType != RenderType::None) {
			if (_renderType >= RenderType::D3D11 && _renderType <= RenderType::D3D12) {
				...
				if (_renderType == RenderType::D3D11) {
					...
					g_methodsTable = (std::uintptr_t*)::calloc(205, sizeof(std::uintptr_t));
					::memcpy(g_methodsTable, *(std::uintptr_t**)swapChain, 18 * sizeof(std::uintptr_t));
					::memcpy(g_methodsTable + 18, *(std::uintptr_t**)device, 43 * sizeof(std::uintptr_t));
					::memcpy(g_methodsTable + 18 + 43, *(std::uintptr_t**)context, 144 * sizeof(std::uintptr_t));
					...
					g_methodsTable = (std::uintptr_t*)::calloc(150, sizeof(std::uintptr_t));
					::memcpy(g_methodsTable, *(std::uintptr_t**)device, 44 * sizeof(std::uintptr_t));
					::memcpy(g_methodsTable + 44, *(std::uintptr_t**)commandQueue, 19 * sizeof(std::uintptr_t));
					::memcpy(g_methodsTable + 44 + 19, *(std::uintptr_t**)commandAllocator, 9 * sizeof(std::uintptr_t));
					::memcpy(g_methodsTable + 44 + 19 + 9, *(std::uintptr_t**)commandList, 60 * sizeof(std::uintptr_t));
					::memcpy(g_methodsTable + 44 + 19 + 9 + 60, *(std::uintptr_t**)swapChain, 18 * sizeof(std::uintptr_t));
				...
				g_methodsTable = (std::uintptr_t*)::calloc(size, sizeof(std::uintptr_t));

				for (int i = 0; i < size; i++) {
					g_methodsTable[i] = (std::uintptr_t)::GetProcAddress(libOpenGL32, methodsNames[i]);
				}
